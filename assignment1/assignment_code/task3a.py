...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray):
    ...
    #print(outputs.shape)
    cost=-np.sum(np.multiply(targets,np.log(outputs)),axis=1)
    #print(cost.shape)
    cel=np.mean(cost)
    #print(cel.shape)
...
class SoftmaxModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        gradient3D=np.zeros((X.shape[0],X.shape[1],outputs.shape[1]))
        for image in range(X.shape[0]):
            gradient=-(np.outer(X[image],(targets[image]-outputs[image])))
            gradient3D[image]=gradient
        self.grad=np.mean(gradient3D, axis=0)
        ...
        self.grad=self.grad +self.l2_reg_lambda*self.w*2
...
def one_hot_encode(Y: np.ndarray, num_classes: int):
    ...
    # TODO implement this function (Task 3a)
    """
    Y_value= [0 for _ in range(num_classes)]
    Y_value[Y[value][0]]=1
    return Y_value
    """
